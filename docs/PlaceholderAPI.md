...
| **玩家临时数据** | `%kamenu_tmpdata_<键名>%` | 数据库 | ✅ 是 | 带到期时间的玩家键值对，到期自动清空 |
...
读取玩家带到期时间的键值对（可由 `tmpdata:` 动作写入）。过期后键会自动清空，适合冷却、限时加成等场景。

**格式：** `%kamenu_tmpdata_<键名>%`

| 变量 | 说明 |
|------|------|
| `%kamenu_tmpdata_<键名>%` | 存储值；不存在或已过期返回 `"null"` |
| `%kamenu_tmpdata_time_<键名>%` | 剩余秒数（纯数字，无单位，方便其他插件直接数值判断）；不存在或已过期返回 `0` |
| `%kamenu_tmpdata_timeformat_<键名>%` | 用户可读文本，如 `1天 2时 30分 15秒`；已过期返回 `0秒` |

**示例：**

| 变量 | 说明 |
|------|------|
| `%kamenu_tmpdata_bonus%` | 读取玩家的 `bonus` 临时数据值 |
| `%kamenu_tmpdata_time_bonus%` | 读取 `bonus` 剩余秒数（如 `86399`） |
| `%kamenu_tmpdata_timeformat_bonus%` | 读取 `bonus` 剩余可读文本（如 `1天 2时 30分 15秒`） |

**注意：**
- 所有数据类型（`data`、`gdata`、`meta`、`list`、`glist` 和 `tmpdata`）的键名都允许包含 `_`，但不建议使用；建议使用 `%kamenu_tmpdata_dailybonus%`，而不是 `%kamenu_tmpdata_daily_bonus%`
- 数据持久化到数据库，玩家下线或服务器重启后仍按原到期时间计算
- 到期后读取立即返回空；数据库中的过期行由周期任务物理清理
