...
每次渲染时，插件会按当前页实际生成的 repeat 按钮数量计算补位。如果数量不能被 `columns` 整除，会在 repeat 按钮末尾添加相应数量的空白按钮。例如 `columns: 3` 且当前页生成 28 个按钮时，会添加 2 个空白按钮，使按钮总数变为 30。空白按钮没有显示文字，但点击后执行 `reset`，用于重新建立当前菜单的回调。上一页/下一页等普通按钮不计入 repeat 数量，并会继续追加在补位按钮之后。
