...
**背景说明：**
服务器与客户端之间的网络通信存在一定的延迟。虽然正常情况下该延迟仅为几十毫秒，但在网络环境较差或服务器负载较高时可能达到 1 秒或更长。在此期间，如果玩家对游戏世界进行操作（如移动物品、丢弃物品等），可能导致菜单逻辑与实际状态不一致，引发异常行为。

`after_action` 参数通过在客户端本地执行遮罩操作，防止玩家在服务器响应期间进行非法操作，确保菜单逻辑的完整性。
...
- **建议使用 `WAIT_FOR_RESPONSE`**：涉及重要操作（如物品交易、权限变更）且需要防止网络延迟不稳定/TPS较低的场景
