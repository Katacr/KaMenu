...
| `refresh`     | 原地刷新 Container 按钮、标题或属性         | ❌ |
...
### refresh - 刷新 Container

原地刷新当前 Container，不重新打开菜单，也不执行 `Events.Open`。裸 `refresh` 或空目标 `refresh:` 刷新全部按钮图标；`refresh: *` 还会刷新标题和容器属性。
...
替换当前菜单的完整参数列表，并立即刷新菜单。执行该动作时会先使用旧参数解析动作文本，再写入新参数；Container 会原地刷新标题、属性和全部按钮，Dialog 会跳过 `Events.Open` 强制重开。
...
清空当前菜单参数，但不主动刷新菜单。需要立即更新按钮显示时，可在其后追加 `reset`（Dialog）或 `refresh`（Container）。
