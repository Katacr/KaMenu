...
import org.katacr.kamenu.dialog.paper.PaperDialogActionFactory
...
object PaperMenuUI {
    ...
    /** 强制重新渲染内存菜单配置，不重复执行 Events.Open。 */
        ...
        val dialogType = when (bottomType) {
                ...
                val exitBtn = bottomSection?.let { section ->
                    ...
                    if (exitText.isNotEmpty()) {
                        ...
                        val builder = ActionButton.builder(TextParser.parseText(exitText, player))
                            .action(PaperDialogActionFactory.build(player, config, "Bottom.exit.actions", inputKeys, inputTypes, inputRemoveChars, checkboxMappings, menuOpener, closesDialogAfterAction, contextId = menuId))
            ...
            "confirmation" -> {
                ...
                val confirmBuilder = ActionButton.builder(TextParser.parseText(confirmBtnText, player))
                    .action(PaperDialogActionFactory.build(player, config, "Bottom.confirm.actions", inputKeys, inputTypes, inputRemoveChars, checkboxMappings, menuOpener, closesDialogAfterAction, contextId = menuId))
                ...
                val denyBuilder = ActionButton.builder(TextParser.parseText(denyBtnText, player))
                    .action(PaperDialogActionFactory.build(player, config, "Bottom.deny.actions", inputKeys, inputTypes, inputRemoveChars, checkboxMappings, menuOpener, closesDialogAfterAction, contextId = menuId))
                ...
                val builder = ActionButton.builder(TextParser.parseText(btnText, player))
                    .action(PaperDialogActionFactory.build(player, config, path, inputKeys, inputTypes, inputRemoveChars, checkboxMappings, menuOpener, closesDialogAfterAction, contextId = menuId))
